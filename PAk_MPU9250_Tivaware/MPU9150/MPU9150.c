...
void ConfigMPU9150() {
	// wake up device
	I2CWriteByte(MPU9150_ADDRESS, PWR_MGMT_1, 0x00); // Clear sleep mode bit (6), enable all sensors
	//SysCtlDelay(8000);//	delay(100); // Wait for all registers to reset
	delayMS(300);

	// get stable time source
	I2CWriteByte(MPU9150_ADDRESS, PWR_MGMT_1, 0x01); // Auto select clock source to be PLL gyroscope reference if ready else
	...
	I2CWriteByte(MPU9150_ADDRESS, CONFIG, 0x03);

	// Set sample rate = gyroscope output rate/(1 + SMPLRT_DIV)
	I2CWriteByte(MPU9150_ADDRESS, SMPLRT_DIV, 0x04); // Use a 200 Hz rate; a rate consistent with the filter update rate
	// determined inset in CONFIG above

	// Set gyroscope full scale range
	// Range selects FS_SEL and AFS_SEL are 0 - 3, so 2-bit values are left-shifted into positions 4:3
	uint8_t c = I2CReadByte(MPU9150_ADDRESS, GYRO_CONFIG);
	//  writeRegister(GYRO_CONFIG, c & ~0xE0); // Clear self-test bits [7:5]
	delayMS(100);
	I2CWriteByte(MPU9150_ADDRESS, GYRO_CONFIG, c & ~0xE0); // Clear Fchoice bits [7:5]
	//I2CWriteByte(MPU9150_ADDRESS, GYRO_CONFIG, c & ~0x02); // Clear Fchoice bits [1:0]
	I2CWriteByte(MPU9150_ADDRESS, GYRO_CONFIG, c & ~0x18); // Clear AFS bits [4:3]
	I2CWriteByte(MPU9150_ADDRESS, GYRO_CONFIG, c | Gscale << 3); // Set full scale range for the gyro

	//c=c & ~0xE0;
	//c=c & ~0x18;
	//c=c | Gscale << 3;
	//I2CWriteByte(MPU9150_ADDRESS, GYRO_CONFIG, c);
	...
	c = I2CReadByte(MPU9150_ADDRESS, ACCEL_CONFIG);
	delayMS(100);
	//  writeRegister(MPUREG_ACCEL_CONFIG, c & ~0xE0); // Clear self-test bits [7:5]
	I2CWriteByte(MPU9150_ADDRESS, ACCEL_CONFIG, c & ~0xE0); // Clear AFS bits [7:5]
	I2CWriteByte(MPU9150_ADDRESS, ACCEL_CONFIG, c & ~0x18); // Clear AFS bits [4:3]
	I2CWriteByte(MPU9150_ADDRESS, ACCEL_CONFIG, c | Ascale << 3); // Set full scale range for the accelerometer

	//c = c & ~0x18;  // Clear AFS bits [4:3]
	//c = c | Ascale << 3; // Set full scale range for the accelerometer
	//I2CWriteByte(MPU9150_ADDRESS, MPUREG_ACCEL_CONFIG, c ); // Set full scale range for the accelerometer
	...
	//c = I2CReadByte(MPU9150_ADDRESS, MPUREG_ACCEL_CONFIG_2);
	//delayMS(100);

	//I2CWriteByte(MPU9150_ADDRESS, MPUREG_ACCEL_CONFIG_2, c & ~0x0F); // Clear accel_fchoice_b (bit 3) and A_DLPFG (bits [2:0])
	//I2CWriteByte(MPU9150_ADDRESS, MPUREG_ACCEL_CONFIG_2, c | 0x03); // Set accelerometer rate to 1 kHz and bandwidth to 41 Hz

	//c = c & ~0x0F; // Clear accel_fchoice_b (bit 3) and A_DLPFG (bits [2:0])
	//  c = c | 0x03;  // Set accelerometer rate to 1 kHz and bandwidth to 41 Hz
	//I2CWriteByte(MPU9150_ADDRESS, MPUREG_ACCEL_CONFIG_2, c);
	delayMS(100);
	...
	I2CWriteByte(MPU9150_ADDRESS, INT_PIN_CFG, 0x22);
	I2CWriteByte(MPU9150_ADDRESS, INT_ENABLE, 0x01); // Enable data ready (bit 0) interrupt

	//SOME TESTS
	//I2CWriteByte(MPU9150_ADDRESS, MPUREG_INT_PIN_CFG, valor_dato);

	//I2CWriteByte(MPU9150_ADDRESS, MPUREG_INT_PIN_CFG, 0x36);
	//I2CWriteByte(MPU9150_ADDRESS, MPUREG_INT_PIN_CFG, 0x16);
	//I2CWriteByte(MPU9150_ADDRESS, MPUREG_INT_ENABLE, 0x09);  // Enable data ready (bit 0) interrupt
...
void CalibrateMPU9150(float * dest1, float * dest2) {
	...
	// reset device
	I2CWriteByte(MPU9150_ADDRESS, PWR_MGMT_1, 0x80); // Write a one to bit 7 reset bit; toggle reset device
	//SysCtlDelay(8000);//delay(100);
	delayMS(500);

	// get stable time source; Auto select clock source to be PLL gyroscope reference if ready
	// else use the internal oscillator, bits 2:0 = 001
	I2CWriteByte(MPU9150_ADDRESS, PWR_MGMT_1, 0x01);
	I2CWriteByte(MPU9150_ADDRESS, PWR_MGMT_2, 0x00);
	//SysCtlDelay(8000);//delay(200);
	delayMS(300);

	// Configure device for bias calculation
	I2CWriteByte(MPU9150_ADDRESS, INT_ENABLE, 0x00); // Disable all interrupts
	I2CWriteByte(MPU9150_ADDRESS, FIFO_EN, 0x00);      // Disable FIFO
	I2CWriteByte(MPU9150_ADDRESS, PWR_MGMT_1, 0x00); // Turn on internal clock source
	I2CWriteByte(MPU9150_ADDRESS, I2C_MST_CTRL, 0x00); // Disable I2C master
	I2CWriteByte(MPU9150_ADDRESS, USER_CTRL, 0x00); // Disable FIFO and I2C master modes
	I2CWriteByte(MPU9150_ADDRESS, USER_CTRL, 0x0C);    // Reset FIFO and DMP
	//SysCtlDelay(800);//delay(15);
	delayMS(100);

	// Configure MPU9250 gyro and accelerometer for bias calculation
	I2CWriteByte(MPU9150_ADDRESS, CONFIG, 0x01); // Set low-pass filter to 188 Hz
	I2CWriteByte(MPU9150_ADDRESS, SMPLRT_DIV, 0x00); // Set sample rate to 1 kHz
	I2CWriteByte(MPU9150_ADDRESS, GYRO_CONFIG, 0x00); // Set gyro full-scale to 250 degrees per second, maximum sensitivity
	I2CWriteByte(MPU9150_ADDRESS, ACCEL_CONFIG, 0x00); // Set accelerometer full-scale to 2 g, maximum sensitivity
	...
	I2CWriteByte(MPU9150_ADDRESS, USER_CTRL, 0x40);   // Enable FIFO
	I2CWriteByte(MPU9150_ADDRESS, FIFO_EN, 0x78); // Enable gyro and accelerometer sensors for FIFO  (max size 512 bytes in MPU-9150)
	//SysCtlDelay(800);//delay(40); // accumulate 40 samples in 40 milliseconds = 480 bytes
	delayMS(100);

	// At end of sample accumulation, turn off FIFO sensor read
	I2CWriteByte(MPU9150_ADDRESS, FIFO_EN, 0x00); // Disable gyro and accelerometer sensors for FIFO
	I2CRead(MPU9150_ADDRESS,FIFO_COUNTH, 2, &data[0]); // read FIFO sample count
	...
	for (ii = 0; ii < packet_count; ii++) {
		...
		I2CRead(MPU9150_ADDRESS, FIFO_R_W, 12, &data[0]); // read data for averaging
	...
	I2CWriteByte(MPU9150_ADDRESS, XG_OFFS_USRH, data[0]);
	I2CWriteByte(MPU9150_ADDRESS, XG_OFFS_USRL, data[1]);
	I2CWriteByte(MPU9150_ADDRESS, YG_OFFS_USRH, data[2]);
	I2CWriteByte(MPU9150_ADDRESS, YG_OFFS_USRL, data[3]);
	I2CWriteByte(MPU9150_ADDRESS, ZG_OFFS_USRH, data[4]);
	I2CWriteByte(MPU9150_ADDRESS, ZG_OFFS_USRL, data[5]);
	...
	I2CRead(MPU9150_ADDRESS, XA_OFFSET_H, 2, &data[0]); // Read factory accelerometer trim values
	accel_bias_reg[0] = (int32_t) (((int16_t) data[0] << 8) | data[1]);
	I2CRead(MPU9150_ADDRESS, XA_OFFSET_L_TC, 2, &data[0]);
	accel_bias_reg[1] = (int32_t) (((int16_t) data[0] << 8) | data[1]);
	I2CRead(MPU9150_ADDRESS, ZA_OFFSET_H, 2, &data[0]);
	...
	I2CWriteByte(MPU9150_ADDRESS, XA_OFFSET_H, data[0]);
	I2CWriteByte(MPU9150_ADDRESS, XA_OFFSET_L_TC, data[1]);
	I2CWriteByte(MPU9150_ADDRESS, YA_OFFSET_H, data[2]);
	I2CWriteByte(MPU9150_ADDRESS, YA_OFFSET_L_TC, data[3]);
	I2CWriteByte(MPU9150_ADDRESS, ZA_OFFSET_H, data[4]);
	I2CWriteByte(MPU9150_ADDRESS, ZA_OFFSET_L_TC, data[5]);
...
void MPU9150SelfTest(float * destination) // Should return percent deviation from factory trim values, +/- 14 or less deviation is a pass
{
	...
	I2CWriteByte(MPU9150_ADDRESS, ACCEL_CONFIG, 0xF0);// Enable self test on all three axes and set accelerometer range to +/- 8 g
	I2CWriteByte(MPU9150_ADDRESS, GYRO_CONFIG,  0xE0); // Enable self test on all three axes and set gyro range to +/- 250 degrees/s
	delayMS(250);

	   rawData[0] = I2CReadByte(MPU9150_ADDRESS, SELF_TEST_X); // X-axis self-test results
	   rawData[1] = I2CReadByte(MPU9150_ADDRESS, SELF_TEST_Y); // Y-axis self-test results
	   rawData[2] = I2CReadByte(MPU9150_ADDRESS, SELF_TEST_Z); // Z-axis self-test results
	   rawData[3] = I2CReadByte(MPU9150_ADDRESS, SELF_TEST_A); // Mixed-axis self-test results
...
void ConfigAK8975(float * destination) {
	// First extract the factory calibration for each magnetometer axis
	int8_t rawData[3];  // x/y/z gyro calibration data stored here
	I2CWriteByte(AK8975A_ADDRESS, AK8975A_CNTL, 0x00); // Power down magnetometer
	SysCtlDelay(800);
	I2CWriteByte(AK8975A_ADDRESS, AK8975A_CNTL, 0x0F); // Enter Fuse ROM access mode
	SysCtlDelay(800);
	I2CRead(AK8975A_ADDRESS, AK8975A_ASAX, 3, &rawData[0]); // Read the x-, y-, and z-axis calibration values
...
int16_t ReadTempData_MPU9150() {
	int8_t rawData[2];  // x/y/z gyro register data stored here
	I2CRead(MPU9150_ADDRESS, TEMP_OUT_H, 2, &rawData[0]); // Read the two raw data registers sequentially into data array
	return ((int16_t) rawData[0] << 8) | rawData[1]; // Turn the MSB and LSB into a 16-bit value
}
void ReadMagData_MPU9150(int16_t * destination) {

	  uint8_t rawData[6];  // x/y/z gyro register data stored here
	  I2CWriteByte(AK8975A_ADDRESS, AK8975A_CNTL, 0x01); // toggle enable data read from magnetometer, no continuous read mode!
	  delayMS(10);
	  ...
	  if( I2CReadByte(AK8975A_ADDRESS, AK8975A_ST1) & 0x01) { // wait for magnetometer data ready bit to be set
		  I2CRead(AK8975A_ADDRESS, AK8975A_XOUT_L, 6, &rawData[0]);  // Read the six raw data registers sequentially into data array
...
void ReadGyroData_MPU9150(int16_t * destination) {
	int8_t rawData[6];  // x/y/z gyro register data stored here
	I2CRead(MPU9150_ADDRESS, GYRO_XOUT_H, 6, &rawData[0]); // Read the six raw data registers sequentially into data array
	destination[0] = (int16_t) (((int16_t) rawData[0] << 8) | rawData[1]); // Turn the MSB and LSB into a signed 16-bit value
	destination[1] = (int16_t) (((int16_t) rawData[2] << 8) | rawData[3]);
	destination[2] = (int16_t) (((int16_t) rawData[4] << 8) | rawData[5]);
}
void ReadAccelData_MPU9150(int16_t * destination) {
	int8_t rawData[6];  // x/y/z accel register data stored here
	I2CRead(MPU9150_ADDRESS, ACCEL_XOUT_H, 6, &rawData[0]); // Read the six raw data registers into data array
	destination[0] = (int16_t) (((int16_t) rawData[0] << 8) | rawData[1]); // Turn the MSB and LSB into a signed 16-bit value
	destination[1] = (int16_t) (((int16_t) rawData[2] << 8) | rawData[3]);
	destination[2] = (int16_t) (((int16_t) rawData[4] << 8) | rawData[5]);
}
void GetMres_MPU9150() {
	switch (Mscale) {
...
float Now_MPU9150() {
		//UARTprintf("delay deltat=%d beta:%d - dif_cycles1:%d\n ",(uint32_t)(deltat*1000),(uint32_t)(beta_p*1000),(cycles1-cycles1_ant));
...
void Setup_MPU9150() {
	...
	while(c!=0x68)
	{
		c = I2CReadByte(MPU9150_ADDRESS, MPUREG_WIA); // Read WHO_AM_I register for MPU-9250
...
//		UARTprintf("%d \n",0x71);
	...
	//if (c == 0x71) // WHO_AM_I should always be 0x68
		if (c == 0x68) // WHO_AM_I should always be 0x68
			{
		UARTprintf("MPU9150 is online...\n");

		MPU9150SelfTest(SelfTest); // Start by performing self test and reporting values
		...
		ConfigMPU9150();
		UARTprintf("MPU9150 initialized for active data mode....\n"); // Initialize device for active mode read of acclerometer, gyroscope, and temperature
		...
		while (d != 0x48) {
			...
			d = I2CReadByte(AK8975A_ADDRESS, WHO_AM_I_AK8975A); // Read WHO_AM_I register for AK8963
			UARTprintf("AK8975 ");
			UARTprintf("I AM ");
			UARTprintf("0x%x", d);
			UARTprintf(" I should be ");
		...
		ConfigAK8975(magCalibration);
		UARTprintf("AK8975 initialized for active data mode....\n"); // Initialize device for active mode read of magnetometer
...
void MadgwickQuaternionUpdate_MPU9150(float ax, float ay, float az, float gx, float gy,
...
// measured ones.
void MahonyQuaternionUpdate_MPU9150(float ax, float ay, float az, float gx, float gy,
		float gz, float mx, float my, float mz) {
	float q1 = q[0], q2 = q[1], q3 = q[2], q4 = q[3]; // short name local variable for readability
	float norm;
	float hx, hy, bx, bz;
...
void UpdateData_MPU9150() {

	bool new_data = false;
	// SerialDebug=0;
	// If intPin goes high, all data registers have new data
	if (I2CReadByte(MPU9150_ADDRESS, INT_STATUS) & 0x01) {

		// On interrupt, check if data ready interrupt
		/***READ ACCELEROMETER***/
		ReadAccelData_MPU9150(accelCount);  // Read the x/y/z adc values
//		GetAres();
		...
		/***READ GYROSCOPE***/
		ReadGyroData_MPU9150(gyroCount);  // Read the x/y/z adc values
		//GetGres();
		...
		/***READ MAGNETOMETER***/
		ReadMagData_MPU9150(magCount);  // Read the x/y/z adc values
		//GetMres();
...
//		magBias[2] = +125.;  // User environmental z-axis correction in milliGauss
	...
	MadgwickQuaternionUpdate_MPU9150(ax, ay, -az, gx * PI / 180.0f, gy * PI / 180.0f,gz * PI / 180.0f, my, mx, mz);
	//MadgwickQuaternionUpdate_MPU9150(ax, ay, -az, gx*PI/180.0f, gy*PI/180.0f, gz*PI/180.0f,  my,  mx, mz);
	//MahonyQuaternionUpdate_MPU9150(ax, ay, az, gx*PI/180.0f, gy*PI/180.0f, gz*PI/180.0f, my, mx, mz);

	//NOT NICE
	//	 roll  = atan2f(ay, az) * RAD_TO_DEG;
	//	 pitch = atanf(-ax / sqrt(ay * ay + az * az)) * RAD_TO_DEG;
